...
download_file() {
    ...
    # 如果文件存在，直接跳过下载
    if [ -f "$dest" ]; then
        echo "✅ $filename already exists in $model_type, skipping download"
        return 0
    fi
...
declare -A downloads=(
    ...
    ["${MODEL_DIR}/loras/Titty_Drop_Wan_2.1_LoRA.safetensors"]="https://huggingface.co/jaysowen/wan_loras/resolve/main/Titty_Drop_Wan_2.1_LoRA.safetensors"
    ["${MODEL_DIR}/loras/wan_female_masturbation.safetensors"]="https://huggingface.co/jaysowen/wan_loras/resolve/main/Wan_Female_Masturbation.safetensors"
